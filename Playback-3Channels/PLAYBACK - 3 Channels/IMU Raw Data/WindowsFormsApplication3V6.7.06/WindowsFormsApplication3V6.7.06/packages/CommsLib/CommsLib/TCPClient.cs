...
namespace CommsLib
{
    public class TCPClient : ClientBase
    {
        ...
        byte[] ServerHandshakeData = new byte[] { 17, 05, 19, 91 };
        public Boolean ServerHandshakeConfirmed = false;
        ...
        public override Boolean ConnectToServer(string serverIP, int port)
        {
            theClient = new TcpClient();
        ...
        */
        ...
        public override void SendData(string message)
        {
            ...
            serverStream.Write(Encoding.ASCII.GetBytes(message), 0, message.Length);
            serverStream.Flush();
        }

        public override bool SendData(CommandID Mode, byte[] Data)
        {
            NetworkStream serverStream = theClient.GetStream();
        ...
        void ProcessPacket(byte[] message)
        {
            ...
            {
                case 1:
                    if (message[1] == ServerHandshakeData[0] && message[2] == ServerHandshakeData[1] && message[3] == ServerHandshakeData[2] && message[4] == ServerHandshakeData[3])
                    {
                        ServerHandshakeConfirmed = true;
                    }
                    break;
                default:
                    _ClientMessageReceivedEvent(message);
                    break;
            }
